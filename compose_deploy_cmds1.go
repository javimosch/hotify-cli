...
func handleDeployCompose() {
	...
	remotePath := cmd.String("remote-path", "", "Override remote destination path (default: app's compose_path or /home/dk1/<id>)")
	...
	if err := client.DeployFolder(*appID, *source, destPath); err != nil {
		printOutput(CommandResult{
			...
			Error: &CommandError{
				...
				Message:     fmt.Sprintf("Failed to copy project tree: %v", err),
				Recoverable: true,
				Suggestions: []string{
...
func handleComposeSyncCLI() {
	...
	if _, statErr := os.Stat(localCompose); statErr == nil {
		if format == OutputFormatText {
			fmt.Printf("Syncing %s → %s:%s/%s\n", cfName, target.Name, app.ComposePath, cfName)
		}
		...
		if err := client.DeployBinary(*appID, localCompose, remoteDest); err != nil {
			printOutput(CommandResult{
				Version: Version, Success: false,
				Error: &CommandError{
					Code: ExitGenericFailure, Type: "sync_error",
					Message:     fmt.Sprintf("Failed to sync compose file: %v", err),
					Recoverable: true,
				},
			}, format)
			os.Exit(ExitGenericFailure)
	...
	if *withEnv {
		...
		if _, statErr := os.Stat(localEnv); statErr == nil {
			if format == OutputFormatText {
				fmt.Printf("Syncing .env → %s:%s/.env\n", target.Name, app.ComposePath)
			}
