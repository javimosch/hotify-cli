...
// DryRunDiff generates the proposed Traefik dynamic YAML and returns a
// unified-diff-style string showing what would change vs the current file.
func DryRunDiff(config *Config) (string, error) {
...
// simpleDiff returns a line-by-line diff with " " (unchanged), "+" (added), "-" (removed).
func simpleDiff(oldStr, newStr string) string {
	oldLines := lineSplit(oldStr)
	newLines := lineSplit(newStr)
	...
	// Reverse because we backtracted from end
